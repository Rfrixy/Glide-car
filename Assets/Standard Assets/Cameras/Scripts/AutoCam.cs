...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] public bool m_FollowVelocity = false;// Whether the rig will rotate in the direction of the target's velocity.
        [SerializeField] public bool m_FollowTilt = true; // Whether the rig will tilt (around X axis) with the target.
        ...
        public float rollUpVal = 0f;
		public float minTilt = -45,maxTilt=45;
		...
		public float rollMultiplier=0.25f;
        protected override void FollowTarget(float deltaTime)
        {
            ...
            if (!m_FollowTilt)
            {
                //Debug.Log(targetRigidbody.velocity.y + "yVel");
                //targetForward.y = Mathf.Clamp ( rollUpVal*targetRigidbody.velocity.y,-0.3f,0.3f);// new

                targetForward.y = 0; // old
            ...
            //			targetForward.y = Mathf.Clamp (targetForward.y, minTilt, maxTilt);
            // and aligning with the target object's up direction (i.e. its 'roll')
            //				m_RollUp = m_RollSpeed > 0 ? Vector3.Slerp(m_RollUp, targetUp, m_RollSpeed*deltaTime) : Vector3.up;

            //			transform.rotation = Quaternion.Lerp(transform.rotation, m_RollUp, m_TurnSpeed*m_CurrentTurnAmount*deltaTime);
            //			m_RollUp = new Vector3 (targetUp.x + rollAmount * rollMultiplier, targetUp.y, targetUp.z);


            if (grounded)
                m_RollUp = new Vector3(0, targetUp.y, 0).normalized;
            else
                m_RollUp = Vector3.up;
			
           	Quaternion rollRotation = Quaternion.LookRotation(targetForward, m_RollUp); // rollup is vector3.up when car is aerial and targetup when car is grounded
